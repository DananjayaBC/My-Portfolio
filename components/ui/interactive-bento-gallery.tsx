      ...
      try {
        if (videoRef.current.readyState >= 3) {
          ...
          await new Promise((resolve) => {
            if (videoRef.current) {
              // Wait until the video can start playing
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              (videoRef.current as any).oncanplay = resolve;
            }
          });
          if (mounted) {
            setIsBuffering(false);
...
const InteractiveBentoGallery: React.FC<InteractiveBentoGalleryProps> = ({
  ...
  useEffect(() => {
    ...
    if (selectedItem) {
      ...
      body.style.top = `-${scrollYBeforeOpenRef.current}px`;
      ...
      const y = top ? -parseInt(top, 10) : scrollYBeforeOpenRef.current;
      // Defer to next paint(s) to avoid jump-to-top when layout recalculates
  ...
  return (
    <div className="w-full mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
      ...
      <AnimatePresence mode="wait">
        {selectedItem ? (
          ...
          <motion.div
            className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 auto-rows-[140px] sm:auto-rows-[100px] md:auto-rows-[80px]"
            initial="hidden"
            animate="visible"
            exit="hidden"
            variants={{
